...
%RUNSCENARIO Execute one isolated, timestamped V5.1 experiment.
arguments
...
[P,selectedParameters] = v51.configureRun(cfg,runDir,backend, ...
    options.WindowsExecutable);
parameterSnapshot = fullfile(runDir,"parameter_snapshot.param");
write_ardupilot_sitl_defaults_v51(P,parameterSnapshot);
...
snapshot = struct("created",string(datetime("now","TimeZone","UTC")), ...
    "version","5.1","backend",backend,"scenario",scenarioName, ...
...
if backend == "wsl"
    ...
    v51.invokePowerShell(startScript,["-RunDirectory",runDir, ...
...
try
    ...
    out = sim(in);
    simulationPass = true;
    v51.writeSimulationData(out,P,double(cfg.simulation.logging_step_s), ...
...
if fid < 0, error("UAVV51:FileWrite","Cannot write %s",filePath); end
...
try
    v51.invokePowerShell(stopScript,["-RunDirectory",runDir,"-Distro",distro]);
catch ME
