...
%ANALYZERUN Layered V5.1 acceptance: infrastructure through response.
arguments
    runDir (1,1) string
    cfg struct
...
elseif ismember("TimeUS",string(data.Properties.VariableNames))
    ...
    error("UAVV51:DataFlashTime","DataFlash CSV has no TimeUS/ap_time_s.");
end
end
...
if isempty(apIndex) || isempty(traceIndices)
    error("UAVV51:NoRCTransition","No RC transition found for time alignment.");
end
...
if bestScore > 20
    error("UAVV51:UnmatchedRCTransition", ...
